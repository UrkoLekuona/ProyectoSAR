...
<body>
  ...
  –––––––––––––––––––––––––––––––––––––––––––––––––– -->
	...
	<div class="nav" style="margin-left: 5%; width: 90%;">
		...
		<a class="active" href="Subir.php">Tus fotos</a>
		<a href="contactos.php">Contacto</a>
	</div>
	<div style="margin-left: 5%; width: 90%; height:300px;">
		<h2>Sube tus fotos</h2></br>
		<form id="subirimagen" action="" method="post" enctype="multipart/form-data">
			<input type='file' name="imagen" accept="image/*" onchange="cargarFoto(this)" onload="this.value=null"></br>
			<p> <img id = "foto" name="foto"> </p>
			<input type='submit' id='subir' value='Subir imagen'></br>
	...
	<div style="margin-left: 5%; width: 90%; height:650px; overflow: scroll;  overflow-x: hidden;">
		<?php
			...
			while (($archivo = $dirint->read()) !== false){
				if(estaenelxml($archivo)){
					echo "<div class='fotoeliminar' style='height:300px; width:300px; display: inline-block; margin: 10px; margin-left:4.8em'>";
			...
			function estaenelxml($archivo){
				$xml = simplexml_load_file('fotos.xml');
				
				$users = $xml->xpath('//user[@email="'. $_SESSION["email"].'"]');
				if (count($users)>=1) {
					$user=$users[0];
					foreach($user->children() as $imagen){
						if(trim($imagen->__toString()) == trim((string)$archivo)){
							return true;
						}
					}
				}
